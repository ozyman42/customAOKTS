...
bool Sheet_HandleCommand(HWND sheet, WORD code, WORD id, HWND control)
{
	...
	case ID_FILE_TRIGREAD:
		...
		break;
	//Here is Alex Leung's Custom event being handled.
	case ID_FILE_CUSTOM:
		try
		{
			customScript(&scen);
			MessageBox(sheet, "Your Custom Script has Succeeded without C++ exceptions", "Scenario Custom Script", MB_ICONWARNING);
		}
		catch (std::exception& ex)
		{
			std::string str ("Your Custom Script has failed.\nError: ");
			str.append(ex.what());
			MessageBox(sheet, str.c_str(), "Scenario Custom Script", MB_ICONERROR);
		}
		break;
